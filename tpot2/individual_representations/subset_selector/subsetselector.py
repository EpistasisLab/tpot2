...
class SubsetSelector(BaseIndividual):
    def __init__(   self,
                    values,
                    rng_, # this being passed means we can control the randomness
                    initial_set = None,
        ...
        if initial_set is None:
            self.subsets = set(rng.choice(values, size=k, replace=False))
        else:
    ...
    def _mutate_add(self,rng_):
        ...
        if len(not_included) > 1:
            self.subsets.update(rng.choice(not_included, k=min(self.k, len(not_included)), replace=False))
            return True
        else:
            return False
    ...
    def _mutate_remove(self,rng_):
        ...
        if len(self.subsets) > 1:
            self.subsets = self.subsets - set(rng.choice(list(self.subsets), k=min(self.k, len(self.subsets)-1), replace=False))
