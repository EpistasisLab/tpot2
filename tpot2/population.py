...
def crossover(parents, rng_):
    rng = np.random.default_rng(rng_)
    parents[0].crossover(parents[1],rng)
    return parents[0]

def mutate_and_crossover(parents, rng_):
    rng = np.random.default_rng(rng_)
    parents[0].crossover(parents[1],rng)
    parents[0].mutate(rng)
    parents[1].mutate(rng)
    return parents
...
def parallel_create_offspring(parents_list, var_op_list, rng_, n_jobs=1):
    ...
    if n_jobs == 1:
        ...
        for parents, var_op in zip(parents_list,var_op_list):
            ...
            if var_op in built_in_var_ops_dict:
                var_op = built_in_var_ops_dict[var_op]
            delayed_offspring.append(dask.delayed(copy_and_change)(parents, var_op)) # TODO: need to create mini rngs to pass to dask
...
def nonparallel_create_offpring(parents_list, var_op_list, rng_, n_jobs=1):
    ...
    for parents, var_op in zip(parents_list,var_op_list):
        ...
        if var_op in built_in_var_ops_dict:
            var_op = built_in_var_ops_dict[var_op]
        offspring.append(copy_and_change(parents, var_op, rng))
...
def copy_and_change(parents, var_op, rng_):
    rng = np.random.default_rng(rng_)
    offspring = copy.deepcopy(parents)
    offspring = var_op(offspring,rng)
    if isinstance(offspring, collections.abc.Iterable):
        offspring = offspring[0]
    return offspring
